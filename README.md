...
캘러는 고정 SHA 대신 이동식 `@v1` 태그를 핀한다. `deploy-notify.yml` 은 렌더
스크립트를 `github.job_workflow_sha`(= **호출 측이 핀한 ref**)로 체크아웃하므로,
SHA 를 핀하면 이 repo 의 수정이 캘러 6개 repo 에 PR 을 올릴 때까지 반영되지
않는다 (2026-09-03 헤드라인 중복 수정이 정확히 이 이유로 한 번 묻혔다).

수정을 배포하는 절차:

    bash tests/run.sh          # 실패 0건 확인
    git tag -f v1 <새 main sha>
    git push -f origin v1

`-f` push 를 쓰는 이유와 그것이 안전한 이유: 알림 job 은 배포 job 의 `needs` 에
들어가지 않으므로(위 설계 원칙), 태그를 옮기는 순간 진행 중인 배포가 깨지지
않는다. 최악의 경우 그 배포의 **알림 한 건**이 새 렌더러로 나가는 것뿐이다.

계약을 깨는 변경(input 이름·필수 secret 추가 등)은 `v1` 을 옮기지 않고 `v2` 를
새로 만든다 — 캘러가 자기 속도로 넘어올 수 있어야 한다.
